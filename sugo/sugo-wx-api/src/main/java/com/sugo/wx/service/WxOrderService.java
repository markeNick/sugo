...
import com.sugo.common.express.dao.ExpressInfo;
import com.sugo.common.notify.NotifyService;
import com.sugo.common.notify.NotifyType;
import com.sugo.common.system.SystemConfig;
import com.sugo.common.task.TaskService;
import com.sugo.common.util.DateTimeUtil;
...
import com.sugo.sql.service.*;
import com.sugo.sql.util.CouponUserConstant;
import com.sugo.sql.util.OrderHandleOption;
import com.sugo.sql.util.OrderUtil;
import com.sugo.wx.task.OrderUnpaidTask;
...
import static com.sugo.wx.util.WxResponseCode.*;
...
public class WxOrderService {
    ...
    public Object list(Integer userId, Integer showType, Integer page, Integer limit, String sort, String order) {
        ...
        List<SugoOrder> orderList = orderService.queryByOrderStatus(userId, orderStatus, page, limit, sort, order);

        List<Map<String, Object>> orderVoList = new ArrayList<>(orderList.size());
        for (SugoOrder o : orderList) {
            ...
            List<SugoOrderGoods> orderGoodsList = orderGoodsService.queryByOid(o.getId());
    ...
    public Object detail(Integer userId, Integer orderId) {
        ...
        List<SugoOrderGoods> orderGoodsList = orderGoodsService.queryByOid(order.getId());
    ...
    public Object submit(Integer userId, String body) {
        ...
        // 收货地址
        SugoAddress checkedAddress = addressService.query(userId, addressId);
        ...
        if (cartId.equals(0)) {
            ...
            SugoCart cart = cartService.findById(cartId);
        ...
        for (SugoCart checkGoods : checkedGoodsList) {
        ...
        if (couponId != 0 && couponId != -1) {
            SugoCoupon coupon = couponVerifyService.checkCoupon(userId, couponId, userCouponId, checkedGoodsPrice);
            if (coupon == null) {
                return ResponseUtil.badArgumentValue();
            }
        ...
        // 添加订单商品表项
        for (SugoCart cartGoods : checkedGoodsList) {
            // 订单商品
            SugoOrderGoods orderGoods = new SugoOrderGoods();
    ...
    public Object payNotify(HttpServletRequest request, HttpServletResponse response) {
        ...
        SugoOrder order = orderService.findBySn(orderSn);
        if (order == null) {
    ...
    public Object goods(Integer userId, Integer orderId, Integer goodsId) {
        ...
        List<SugoOrderGoods> orderGoodsList = orderGoodsService.findByOidAndGid(orderId, goodsId);
        ...
        SugoOrderGoods orderGoods = orderGoodsList.get(0);
    ...
    public Object comment(Integer userId, String body) {
        ...
        SugoOrderGoods orderGoods = orderGoodsService.findById(orderGoodsId);
        if (orderGoods == null) {
            return ResponseUtil.badArgumentValue();
        }
        ...
        // 1. 创建评价
        SugoComment comment = new SugoComment();
