...
import com.sugo.common.util.bcypt.BCryptPasswordEncoder;
...
import static com.sugo.admin.util.AdminResponseCode.*;
...
public class AdminAdminController {
    ...
    public Object list(String username,
        ...
        List<SugoAdmin> adminList = adminService.querySelective(username, page, limit, sort, order);
        return ResponseUtil.okList(adminList);
    }

    private Object validate(SugoAdmin admin) {
    ...
    @PostMapping("/create")
    public Object create(@RequestBody SugoAdmin admin) {
        ...
        List<SugoAdmin> adminList = adminService.findAdmin(username);
        if (adminList.size() > 0) {
    ...
    public Object read(@NotNull Integer id) {
        SugoAdmin admin = adminService.findById(id);
        return ResponseUtil.ok(admin);
    }
    ...
    @PostMapping("/update")
    public Object update(@RequestBody SugoAdmin admin) {
        Object error = validate(admin);
        if (error != null) {
            return error;
        }
    ...
    @PostMapping("/delete")
    public Object delete(@RequestBody SugoAdmin admin) {
        Integer anotherAdminId = admin.getId();
        if (anotherAdminId == null) {
            return ResponseUtil.badArgument();
        }
        ...
        SugoAdmin currentAdmin = (SugoAdmin) currentUser.getPrincipal();
