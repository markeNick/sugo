...
import com.sugo.sql.service.AdminService;
import com.sugo.sql.service.NoticeAdminService;
import com.sugo.sql.service.NoticeService;
...
import static com.sugo.admin.util.AdminResponseCode.NOTICE_UPDATE_NOT_ALLOWED;
...
public class AdminNoticeController {
    ...
    public Object list(String title, String content,
        ...
        List<SugoNotice> noticeList = noticeService.querySelective(title, content, page, limit, sort, order);
        return ResponseUtil.okList(noticeList);
    }

    private Object validate(SugoNotice notice) {
    ...
    private Integer getAdminId(){
        ...
        SugoAdmin admin = (SugoAdmin) currentUser.getPrincipal();
        return admin.getId();
    }
    ...
    @PostMapping("/create")
    public Object create(@RequestBody SugoNotice notice) {
        ...
        // 2. 添加管理员通知记录
        List<SugoAdmin> adminList = adminService.all();
    ...
    public Object read(@NotNull Integer id) {
        SugoNotice notice = noticeService.findById(id);
        List<SugoNoticeAdmin> noticeAdminList = noticeAdminService.queryByNoticeId(id);
    ...
    @PostMapping("/update")
    public Object update(@RequestBody SugoNotice notice) {
        Object error = validate(notice);
        if (error != null) {
            return error;
        }
        SugoNotice originalNotice = noticeService.findById(notice.getId());
    ...
    @PostMapping("/delete")
    public Object delete(@RequestBody SugoNotice notice) {
        // 1. 删除通知管理员记录
